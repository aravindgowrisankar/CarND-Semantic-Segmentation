...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    output=tf.layers.conv2d_transpose(conv1x1,num_classes,4,2,
                                      padding="same",
                                      kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ...
    layer4_skip = tf.layers.conv2d_transpose(vgg_layer4_out, num_classes, 4, 2,
                                             kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    output = tf.add(output, layer4_skip)

    layer3_skip = tf.layers.conv2d_transpose(vgg_layer3_out, num_classes, 16, 8,
                                             kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    output = tf.add(output, layer3_skip)
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        for image,labels in get_batches_fn(batch_size):
            my_dict={input_image:image,
                     correct_label:labels,
                     keep_prob:0.5}
            
            sess.run(train_op,feed_dict=my_dict)
...
def run():
    ...
    #input_image = tf.placeholder("float", [None, None, None, None])
    correct_label = tf.placeholder("float", [None, None,None,2])
    ...
    with tf.Session() as sess:
        ...
        input_tensor,keep_prob,layer_3,layer_4,layer_7=load_vgg(sess, vgg_path)
